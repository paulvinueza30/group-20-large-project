...
export const editFlashcard = async (req, res) => {
    try {
        ...
        const user = req.user;
        const userId = user._id;
        const updatedFlashcard = await flashcard.findOneAndUpdate({ _id: id, userId }, { frontSide, backSide, editedAt: Date.now() }, { new: true });
        if (!updatedFlashcard) {
            res
                .status(404)
                .json({
...
export const getNextFlashcard = async (req, res) => {
    const categoryId = req.params.categoryId;
    const user = req.user;
    const userId = user._id;
    try {
